...
for(mm in  1:Clen ){
  ...
  BodySite[mm]=paste0(splitname[[mm]][3], splitname[[mm]][4])
...
png(filename=paste0(filename,"shannon.Swabber.Site.wilcox.png"), height = 600, width=600)
bxp <- ggboxplot(mydata, x = "Collect", y = "shannon", color = "Collect", facet.by = "BodySite")
...
png(filename=paste0(filename,".bray-Curtis-cluster.png"), width=800, height=800)
