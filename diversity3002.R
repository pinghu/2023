...
filename<- args[1]
filename="GSS3002.metaphlan.estimated_count.filtered.7"
...
#unbias.simp
mydata1=data.frame(shannon,simp,invsimp,  observed, Menhinick_index, Margalef_index, BodySite,  Collect, id2, Sid, SiteCollect, SiteCollectSid, SidCollect,  NewID, Visit, BodySiteSid)

mydata=mydata1[ BodySite != "Blank",]                   
...
stat.test <- mydata %>%
  ...
  mutate(y.position = 3.2)
bxp <- ggboxplot(mydata, x = "BodySite", y = "shannon", color = "BodySite")
...
bxp <- ggboxplot(mydata, x = "Collect", y = "shannon", color = "Collect", facet.by = "BodySite")
bxp + stat_compare_means(comparisons = list(c("Expert", "Mom")), method = "t.test")
dev.off()

png(filename=paste0(filename,"shannon.Collect.Site.wilcox.png"), height = 600, width=600)
bxp <- ggboxplot(mydata2, x = "Collect", y = "shannon", color = "Collect", facet.by = "BodySite")
bxp + stat_compare_means(comparisons = list(c("Expert", "Mom")), method = "wilcox")
...
bxp <- ggboxplot(mydata, x = "BodySite", y = "observed", color = "BodySite")
bxp + stat_pvalue_manual(stat.test,   label = "p", tip.length = 0.01, step.increase = 0.08)
dev.off()
...
bxp <- ggboxplot(mydata, x = "Collect", y = "observed", color = "Collect", facet.by = "BodySite")
